...
const getQuotes = async (metadata, dbCurrencies, currencies) => {
  const dateNow = new Date()
  const minutesBetweenUpdates = 1
  const toBeUpdated = dbCurrencies.filter(
    (currency) =>
      !currency.quote ||
  ...
  const formattedData = {}
  // check if there is at least one token to be updated, or if one token is not inside database
  if (toBeUpdated.length || dbCurrencies.length != currencies.length) {
    ...
    metadata.forEach((token) => {
      formattedData[token.symbol] = dbCurrencies.find(
        (currency) => currency.symbol === token.symbol
      ).quote
    })
  }
...
export default function useCurrenciesData(currencies: Currency[]): Currency[] {
  ...
  const getData = async () => {
    ...
    const quotes = await getQuotes(metadata, dbCurrencies, currencies)
    console.log("dbCurrencies", dbCurrencies)
